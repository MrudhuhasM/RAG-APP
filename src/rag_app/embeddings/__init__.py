...
def get_chunk_embeddings() -> BaseEmbedding:
    if settings.embedding.provider == "openai":
        ...
        return OpenAIEmbedding(model=settings.openai.embedding_model, api_key=settings.openai.api_key)
    elif settings.embedding.provider == "gemini":
        logger.info("Using llama-index Gemini Embedding Model")
        return GeminiEmbedding(model=settings.gemini.embedding_model, api_key=settings.gemini.api_key)
    elif settings.embedding.provider == "local":
        logger.info("Using llama-index Local Embedding Model")
        return OpenAIEmbedding(api_base=settings.local_models.embedding_base_url, model="text-embedding-3-small")
    else:
